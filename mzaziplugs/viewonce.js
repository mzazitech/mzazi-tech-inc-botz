...
let handler = async (m, { mzazi, trashown, reply }) => {
  ...
  try {
    ...
    if (isViewOnceImage) {
      ...
      await mzazi.sendMessage(m.chat, { image: buffer, caption: `*Retrieved by Trashcore*\n\n*ViewOnce:* Image\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
      return;
    }
    ...
    if (isViewOnceVideo) {
      ...
      const caption = mediaMessage.caption || '';
      await mzazi.sendMessage(m.chat, { video: fs.readFileSync(tempFile), caption: `*𝐎𝐏𝐄𝐍𝐄𝐃 𝐁𝐘 𝐌𝐎𝐍𝐄𝐘𝐇𝐄𝐈𝐒𝐓 𝐀𝐈*\n\n*ViewOnce* Video\n${caption ? `Caption: ${caption}` : ''}` }, { quoted: m });
