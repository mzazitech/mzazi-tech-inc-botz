...
let trashplug = async (m, { text,reply,mzazi,prefix,command }) => {
    ...
    fetch(`https://api.tiklydown.eu.org/api/download/v5?url=${encodeURIComponent(text)}`)
        ...
        const title = `*Downloaded by ${botname}*`
            ...
            mzazi.sendMessage(m.chat, { caption: title, video: { url: videoUrl }}, { quoted: m });
            mzazi.sendMessage(m.chat, { audio: { url: audioUrl }, mimetype: 'audio/mp4' }, { quoted: null });
        })
