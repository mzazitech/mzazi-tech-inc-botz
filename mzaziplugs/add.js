...
let trashplug = async (m, {command, prefix,isAdmins,trashown,reply,text,mzazi }) => {
				...
				if (!text && !m.quoted) {
					...
					try {
						await mzazi.groupParticipantsUpdate(m.chat, [numbersOnly], 'add')
							.then(async (res) => {
								for (let i of res) {
									let invv = await mzazi.groupInviteCode(m.chat);
									...
									if (i.status == 403) {
										...
										await mzazi.sendMessage(`${numbersOnly ? numbersOnly : creator}`, { 
