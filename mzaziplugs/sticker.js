...
let trashplug = async (m, { mzazi, prefix,command,reply,text,mime,quoted,downloadContentFromMessage}) => {
if (!quoted) return reply(`reply Video/Image with Caption ${prefix + command}`)
if (/image/.test(mime)) {
let media = await quoted.download()
let encmedia = await mzazi.sendImageAsSticker(m.chat, media, m, {
...
let media = await quoted.download()
let encmedia = await mzazi.sendVideoAsSticker(m.chat, media, m, {
packname: global.packname,
author: global.author
})
} else {
