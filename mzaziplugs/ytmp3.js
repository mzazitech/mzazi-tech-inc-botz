...
let trashplug = async (m, { mzazi, args, command,reply }) => {
    try {
        ...
        await mzazi.sendMessage(m.chat, { document: { url: downloadURL }, fileName: filename, mimetype: format === 'mp4' ? 'video/mp4' : 'audio/mp3' }, { quoted: m })      
    } catch (e) {
        reply(e.message)
    }
}
